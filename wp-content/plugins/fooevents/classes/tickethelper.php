...
class FooEvents_Ticket_Helper {
    ...
    public function __construct($config) {
        ...
        add_action('wp_ajax_fooevents_validate_new_user', array($this, 'validate_new_user'));
    ...
    public function add_admin_columns($columns) {
        ...
        $columns = array(
            ...
            'Order'             => __('Order', 'woocommerce-events'),
    ...
    public function add_tickets_meta_ticket_details() {
        ...
        $WooCommerceEventsTimeZone              = get_post_meta($WooCommerceEventsProductID, 'WooCommerceEventsTimeZone', true);
        ...
        $WooCommerceEventsZoomText              = $this->ZoomAPIHelper->get_ticket_text($WooCommerceEventsProductID, 'admin');
        $WooCommerceEventsTicketDisplayPrice    = get_post_meta($WooCommerceEventsProductID, 'WooCommerceEventsTicketDisplayPrice', true);
        $WooCommerceEventsTicketDisplayZoom     = get_post_meta($WooCommerceEventsProductID, 'WooCommerceEventsTicketDisplayZoom', true);
        ...
        $checkinTimes = $this->get_checkin_times($post->ID);
        ...
        if (!file_exists($this->Config->barcodePath.$WooCommerceEventsTicketHash.'-'.$WooCommerceEventsTicketID.'.png')) {
        ...
        //generate ics
        $this->ICSHelper->generate_ICS($WooCommerceEventsProductID, $WooCommerceEventsAttendeeEmail);
        $this->ICSHelper->save($WooCommerceEventsTicketID);
        ...
        if (is_object($post) && isset($_POST)) {
            ...
            if($post->post_type == "event_magic_tickets") {
                ...
                if (($this->is_plugin_active('fooevents_multi_day/fooevents-multi-day.php') || is_plugin_active_for_network('fooevents_multi_day/fooevents-multi-day.php')) && $WooCommerceEventsNumDays > 1) {
                    ...
                    if($_POST['WooCommerceEventsStatus'] == 'Checked In') {
    ...
    /**
     * Check if user email or display name already exists
     * 
     */
    public function validate_new_user() {
        ...
        if(in_array($_POST['display_name'], $usernames)) {
        ...
        if (is_object($post) && isset( $_POST )) {
            ...
            if( $post->post_type == "event_magic_tickets" && !empty($_POST['add_ticket']) ) {
                ...
                if(empty($_POST['WooCommerceEventsClientID'])) {
                    ...
                    if(in_array($_POST['WooCommerceEventsPurchaserEmail'], $userEmails)) {
    ...
    public function resend_ticket($postID) {
        ...
        $mailStatus = $this->MailHelper->send_ticket($to, $subject, $ticketBody, $attachments);
    ...
    public function get_ticket_data($ticketID) {
        ...
        $ticket['WooCommerceEventsTicketAddCalendarReminders']   = get_post_meta($WooCommerceEventsProductID, 'WooCommerceEventsTicketAddCalendarReminders', true);
        $ticket['WooCommerceEventsTicketAttachICS']              = get_post_meta($WooCommerceEventsProductID, 'WooCommerceEventsTicketAttachICS', true);
	...
	if(!empty($ticket['WooCommerceEventsTicketHeaderImage'])) {
        ...
        //generate ics
        $this->ICSHelper->generate_ICS($WooCommerceEventsProductID, !empty($ticket['WooCommerceEventsAttendeeEmail']) ? $ticket['WooCommerceEventsAttendeeEmail'] : '');
