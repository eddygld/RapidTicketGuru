...
class FooEvents_Mail_Helper {
     ...
     */
    public function send_ticket($to, $subject, $body, $attachments = array()) {
        ...
        $sendMail = wp_mail($to, $subject, $body, $headers, $attachments);

        if($globalWooCommerceEventsEmailTicketAdmin === "yes") {
            
            $admin_email = get_option('admin_email', true);
            $sendMail = wp_mail($admin_email, $subject, $body, $headers, $attachments);
