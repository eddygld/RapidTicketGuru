...
class FooEvents_Calendar {
    ...
    public function add_plugin_links($links) {
        ...
        $linkSettings = '<a href="admin.php?page=fooevents-settings&tab=calendar">'.__('Settings', 'fooevents-calendar').'</a>'; 
    ...
    public function include_scripts(){
        ...
        wp_enqueue_script('fooevents-calendar-moment',  $this->Config->scriptsPath . 'moment.min.js', array('jquery'), '1.0.0');
        wp_enqueue_script('fooevents-calendar-full-callendar',  $this->Config->scriptsPath . 'fullcalendar.min.js', array('jquery'), '1.0.0');
        wp_enqueue_script('fooevents-calendar-full-callendar-locale',  $this->Config->scriptsPath . 'locale-all.js', array('jquery'), '1.0.0');
    ...
    public function register_scripts() {
        ...
        wp_enqueue_script('wp-color-picker');
        wp_enqueue_script('fooevents-calendar-admin-script', $this->Config->scriptsPath . 'calendar-admin.js', array('jquery', 'jquery-ui-datepicker', 'wp-color-picker'), '1.0.1', true);
    ...
    public function register_styles() {
        ...
        wp_enqueue_style('fooevents-calendar-admin-style',  $this->Config->stylesPath . 'calendar-admin.css', array(), '1.0.0');
    ...
    public function add_admin_menu() {
        ...
        foreach($menu as $item) {
            ...
            if(strtolower($item[0]) == strtolower('fooevents')) {
    ...
    public function process_eventbrite($post_id) {
        ...
        if(isset($_POST['WooCommerceEventsTimeZone'])) {
            ...
            $timezone = sanitize_text_field($_POST['WooCommerceEventsTimeZone']);
        ...
        if(empty($WooCommerceEventsEventbriteID)) {
            ...
            $resp = $client->post_events($event_params, $user['id']);
            
        } else {
            
            //post_events has been modified to new endpoint
            $resp = $client->post_event($WooCommerceEventsEventbriteID, $event_params, $user['id']); 
    ...
    public function get_non_product_events() {
        ...
        array_push($globalFooEventsCalendarPostTypes, 'fe_eventbrite_event');
                ...
                if($multi_day_type == 'select') {
                    ...
                    if(!empty($multi_day_dates)) {
                        ...
                        foreach($multi_day_dates as $date) {
                            ...
                            if ($this->is_plugin_active('woocommerce/woocommerce.php') || is_plugin_active_for_network('woocommerce/woocommerce.php')) {
                    ...
                    if(!empty($event_end_date)) {
                        ...
                        $event_end_date_formatted = $Fooevents_Multiday_Events->format_end_date($event->ID);
